...
def generate_replayed_human_voice(sample_rate=16000, duration=3.5):
    """Scenario 2: Speaker-replayed human voice (human speech rhythm/pitch + speaker bandpass filter & room echo phase_var > 2.5)."""
    ...
    # Speaker frequency bandpass (250Hz - 5000Hz) & mild hiss
...
def generate_ai_tts_source1_elevenlabs(sample_rate=16000, duration=3.5):
    ...
    t = np.linspace(0, duration, int(sample_rate * duration))
    f0 = 135.0  # Completely flat robotic pitch F0
    phase = 2 * np.pi * f0 * t
    voiced = np.sin(phase) + 0.4 * np.sin(2 * phase) + 0.2 * np.sin(3 * phase)
    
    # Syllables with speech timing
...
def generate_ai_tts_source2_wavenet(sample_rate=16000, duration=3.5):
    ...
    t = np.linspace(0, duration, int(sample_rate * duration))
    f0 = 140.0 + 0.5 * np.sin(2 * np.pi * 0.2 * t)  # Almost flat pitch contour
    phase = 2 * np.pi * np.cumsum(f0) / sample_rate
...
def generate_ai_tts_source3_replayed_ai(sample_rate=16000, duration=3.5):
    ...
    # Played through speaker with room echo
...
if __name__ == "__main__":
    print("\n==================================================================")
    print("  DHVANI RAKSHAK - CONSOLIDATED 6-SCENARIO VERIFICATION BENCHMARK  ")
    print("==================================================================\n")
    ...
    for r in results:
        ...
        if st == "NO_SPEECH_DETECTED" or al == "NO_SPEECH_DETECTED":
            ...
            if sc.startswith("1.") or sc.startswith("2."):
                passed = (al == "GREEN" and auth >= 70.0)
            elif sc.startswith("4.") or sc.startswith("5.") or sc.startswith("6."):
                passed = (al == "RED" and auth <= 30.0)
            else:
                passed = False
    ...
    print("=" * 108)
    if passed_all:
        print("🎉 ALL 6 BENCHMARK SCENARIOS PASSED TOGETHER IN THE SAME TEST RUN!")
    else:
