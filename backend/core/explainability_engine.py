...
class ExplainabilityEngine:
    def generate_explanation(self, acoustic_res: dict, prosody_res: dict, speaker_res: dict, risk_res: dict) -> dict:
        ...
        if tts_sigs.get("ElevenLabs", 0.0) > 0.60:
            reasons.append("High correlation with ElevenLabs Neural Vocoder signature (synthetic high-frequency roll-off above 6.5kHz).")
        ...
        if tts_sigs.get("OpenAI_Voice", 0.0) > 0.60:
            reasons.append("High correlation with OpenAI Voice Engine signature (abnormally uniform harmonic spacing).")
            key_factors.append({"feature": "OpenAI Voice Engine Signature", "impact": "HIGH", "value": f"{tts_sigs['OpenAI_Voice']*100:.0f}% Match"})

        if phase_var < 2.0:
            reasons.append(f"Unnatural phase smoothness variance ({phase_var:.2f} vs human baseline > 2.5), indicative of neural phase alignment.")
        ...
        if jitter < 0.12:
            reasons.append(f"Robotic vocal stability: Pitch Jitter is unnaturally low ({jitter:.2f}% vs human baseline 0.20%-1.20%).")
            key_factors.append({"feature": "Pitch Jitter Perturbation", "impact": "CRITICAL", "value": f"{jitter:.2f}%"})

        if std_f0 < 8.0:
            reasons.append(f"Monotone Pitch Contour: F0 pitch variation is flat ({std_f0:.1f}Hz vs human dynamic range > 18Hz).")
